/**
 * Package-owned durable memory-write invariants.
 *
 * 停用说明（B2 决策 2026-08-20）：审计事件 `memory/write` 已不再写入会话日志——
 ...
 * 会被静默放行，新日志不再产生该类事件。
 * @module @dsh-cortex/dsh-memory-harness/invariant
 */
...
/** 事件校验（B2 停用）：memory/write 不再产生，历史 ignorable 事件静默放行。 */
